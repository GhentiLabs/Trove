...
- **chunk-index** — `chunks`, `blobs`, `chunk_locations`, `refcounts`. High write + GC churn. Isolated so GC sweeps don't contend with sync metadata; can use a different engine if measurement favors it (§14.2).
...
- Refcount `chunk_id`s across all *retained* snapshots + current manifests. A chunk is deletable only when refcount hits zero.
- GC is a low-priority background mark-and-sweep: mark from snapshot roots + current files, sweep unreferenced pack entries. Coordinates with in-flight writes via the chunk-index transaction boundary — never sweep a chunk being written or referenced by an in-progress transfer.
...
**M2 — Model + local state.** L2 + L3: manifests, snapshots, version vectors, scanner (watch+rescan), namespaced DBs, GC, quota accounting. Single machine. *Accept:* edit a large file → only changed chunks change; snapshots share chunks; GC reclaims after pruning; bounded queues hold RAM flat under a large tree.

**M3 — Transport + discovery + protocol handshake.** L5+L6+L7 through `Active`: QUIC mTLS pinning, Trove announce/lookup/signal, holepunch, `Hello`/`NetworkConfig`. *Accept:* two real NATed machines establish a direct, pinned, authenticated session via Trove.

**M4 — One-way sync (first shippable end-to-end).** L8 in one-way mode + L4 membership (explicit network object, signed roster, gossip) + multi-source scheduler + crash-safe apply + SyncReceipts. Owner → N replicas, pull-only. *Accept:* a snapshot converges across ≥3 peers including one that was offline; multi-source pull verified; receipts recorded; conflict code present but unreachable. **This is the first real, usable, testable system.**

**M5 — Bidirectional sync.** Light up the dormant path: version-vector reconciliation in both directions, conflict detection, keep-both resolution, tombstones. *Accept:* concurrent offline edits converge to deterministic conflict copies on all nodes; deletes don't resurrect.
...
**M8 — Surface & hardening.** Web UI (P2P-pure), Linux support, parallel-transfer tuning, then the deterministic simulation harness (§13.4).
