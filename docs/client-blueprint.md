...
**M5 — Bidirectional sync.** Build and enable the bidirectional path (the conflict machinery is not pre-built — see M4 deviation 2): replicas become writers, version-vector reconciliation in both directions, conflict detection on concurrent vectors, keep-both resolution, two-way tombstones. In the folder-as-group model this is the `writer` tier doing real two-way work. *Accept:* concurrent offline edits converge to deterministic conflict copies on all nodes; deletes don't resurrect.
